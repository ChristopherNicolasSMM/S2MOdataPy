...
def build_metadata_xml(entities: list) -> str:
    ...
    for entity_cls in entities:
        entity_type_elem = ET.SubElement(schema, "edm:EntityType", {"Name": entity_cls.__name__})
        ...
        for column in mapper.columns:
            ...
            if not column.nullable and not _is_pk(column):
                field_ctrl = ET.SubElement(prop_elem, "Annotation", {"Term": "common.FieldControl"})
                ET.SubElement(
                    field_ctrl, "EnumMember", {"EnumMember": "common.FieldControlType/Mandatory"}
                )
        ...
        if "ListView" in ui_ann:
            ...
            for col in lv.get("columns", []):
                ...
                ET.SubElement(
                    record, "PropertyValue", {"Property": "Label", "String": col["label"]}
                )
                ET.SubElement(record, "PropertyValue", {"Property": "Value", "Path": col["name"]})
                if col.get("sortable"):
                    ET.SubElement(record, "PropertyValue", {"Property": "Sortable", "Bool": "true"})
        ...
        for group in ui_ann.get("FieldGroups", []):
            ...
            ET.SubElement(record, "PropertyValue", {"Property": "Name", "String": group["name"]})
            ET.SubElement(record, "PropertyValue", {"Property": "Label", "String": group["label"]})
