...
class ODataAnnotationParser:
    ...
    def get_all_entities(self) -> List[str]:
        ...
        if self._root is not None:
            return [et.get("Name") for et in self._root.findall(".//edm:EntityType", self._NS)]
        return []
    ...
    def _parse_entity_from_dict(self, entity_name: str) -> UIAnnotations:
        ...
        entity_data = next((e for e in entities if e.get("name") == entity_name), None)
        ...
        # Campos filtráveis
        filters = [
            p["name"] for p in entity_data.get("properties", []) if "String" in p.get("type", "")
        ]
    ...
    def _parse_entity_from_xml(self, entity_name: str) -> UIAnnotations:
        ...
        entity_type = self._root.find(f".//edm:EntityType[@Name='{entity_name}']", self._NS)
